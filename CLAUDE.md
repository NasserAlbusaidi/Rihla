...
- Name-based members: the creator adds **only themselves** (`createGroup` writes a single member doc); every other member self-joins via invite code and **free-types** their own display name. There is **no** "creator adds friends' names" and **no** "pick an unclaimed name" picker (D-12), so no uniqueness guard: duplicate display names are possible; the disambiguator (#196) is wired into only 2 of ~6 surfaces (#279 prevention, #289 display). The dormant `GroupMember.isShadow` field has no creation path — adding-friends-by-name is unbuilt (#278). **Member doc keying is INCONSISTENT: joiners + recovery-copies key by `{uid}`, but the CREATOR's doc is keyed by a random `uuid` with `userId:uid` as a field — so always match members by the `userId` field, never the doc id.** The `.doc(uid)`/`id===uid` lookups in `deleteAccount` previously MISSED creator docs (PII-residue + broken creator recovery) — #294. Display name mirrored from `settingsProvider.deviceName`.
