...
- **Don't `.autoDispose` the ledger/balance providers to "fix" listener leaks (#104).** The always-mounted home dashboard pins them session-long: `BalanceHeroCard → crossGroupBalanceProvider` (all groups) + `activeJourneysProvider` (active groups) → `groupBalancesProvider` → `eventExpensesProvider`/`eventSettlementsProvider` leaves (`group_balance_provider.dart:143-144,476,513`; both roots are plain non-autoDispose `Provider`s). A provider with a live watcher never autoDisposes, so the annotation is a no-op (empirically confirmed in #104). The leak is eager fan-out, not browse-growth (event-detail shares the leaf via `EventRef` record equality). Real lever = lazy/on-demand home aggregation, not `.autoDispose`.
