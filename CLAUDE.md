...
- **On Flutter ≥3.41 a `SnackBar` with an `action` defaults to `persist: true` — it NEVER auto-dismisses (#411).** The SDK derives `persist = persist ?? action != null`; the timeout timer fires without hiding. Any action-bearing snackbar must pass `persist: false` + explicit `duration` (blocked leave/remove sites use 8s). Related: `darkTheme` has **no** `snackBarTheme` — only `lightTheme` sets `behavior: floating`, so dark-mode snackbars render M3 `fixed` (full-width) unless `behavior` is set per call site (the ad-hoc `behavior: floating` overrides are load-bearing in dark mode, not redundant).
