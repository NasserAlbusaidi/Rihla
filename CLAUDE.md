...
- Onboarding: **removed (#56)** — `OnboardingScreen` is deleted, the router has no `/onboarding` route, and `appRouteRedirect` only maps splash→home (it does **not** gate on `onboardingComplete`). The intent is pinned by `test/unit/app_router_test.dart` ("onboarding route is not part of the shippable route tree"). The `onboardingComplete` flag in `AppSettings` is retained only so legacy installs can read it. Don't re-wire onboarding into the route tree without reopening that decision.
