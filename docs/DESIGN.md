...
| D5a | **App defaults to light, not system (2026-07-03, #818).** Until D5's dark pass ships, system-dark users would otherwise land on the undesigned stub by default. | `AppSettings.themeMode` default + `SettingsService.loadSettings` fallbacks (`AppThemeMode.light`). | Revert to `AppThemeMode.system` default once the dark pass (D5) ships. |
