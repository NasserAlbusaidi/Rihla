...
|---|---|---|---|
| D1 | **Token adoption ~48%.** ~90 `lib` files hardcode spacing/radius/text. | 82/172 files use `context.*`; 160 literal `SizedBox`, 141 literal `BorderRadius.circular`, 89 raw `TextStyle(`. | Phased per-feature refactor → tokens/primitives. **All UI features ✅: home #310 · ledger #312 · groups #314 · events #315 · settings/activity/auth #316** (no-op spacing/radius passes shipped). Remaining: cross-cutting `TextStyle→AppTypography` + off-scale/legacy-radius reconcile (visual, golden-reviewed). |
...
| D5 | **✅ Resolved (#900 PR-4).** Dark theme was compiling-but-untuned; now tuned night-navigation: card surface lifted to `#1E2422` for tint-based elevation, the five component themes light already had (chip/divider/bottomSheet/dialog/FAB) ported to dark tokens, input hints moved off `textMuted` (~3.1:1) onto `textSecondary` (~5.6:1). | `color_tokens.dart` dark instance + docstring; `app_theme.dart` `_buildDarkTheme`. | Done. |
| D5a | **✅ Resolved (#900 PR-4).** App defaulted to light, not system, since 2026-07-03 (#818) so system-dark users wouldn't land on the undesigned D5 stub by default. Now that D5's dark pass has shipped, the default reverted to `AppThemeMode.system`. | `AppSettings.themeMode` default + `SettingsService.loadSettings` fallbacks (`AppThemeMode.system`). | Done. |
