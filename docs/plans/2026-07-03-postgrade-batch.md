...
2. Every metadata param read: `value is String && value.isNotEmpty` — the #814 rules floor types 11 named keys but leaves `eventId`/`expenseId` OPAQUE, and legacy pre-#814 docs have no floor. Copy the `_metadataString` pattern (`activity_display.dart:32-35`, file-private — export it or add a sibling helper). **Gate r1 P2: `_metadataString` itself checks only `is String` — a verbatim reuse returns `''` for an empty metadata value and builds a malformed `/group/:gid/event//ledger` (empty path segment); the deep-link read must ADD `.isNotEmpty` (sibling helper or inline), never bare-reuse the display helper.** Guard failure → group-detail push.
...
RED-first tests (follow the `*_navigation_test.dart` convention): per-type route assertions (harness fake router currently defines only /home, /activity, /group/:id — `cross_group_activity_screen_test.dart:137-158` — add the target routes; **Gate r1 P3: the fake router's param is `:id` while the real router uses `:gid` — keep the fake self-consistent when adding the nested target routes; pushes are literal path strings so prod is unaffected**); REWRITE the pinned uniform-push test (:605); forged-metadata table: `eventId` as int/bool/map → group detail + NO ErrorWidget; **empty-string eventId → group detail (the Gate r1 P2 case)**; absent eventId → group detail. Commit: `feat(activity): History rows deep-link to what they describe (#840)` + `Refs #840` + `Spec:` line → this doc §PR-7.
