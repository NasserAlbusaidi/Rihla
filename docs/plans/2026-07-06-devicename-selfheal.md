...
- `surfaceRecoveryOutcome` already takes `prefs`; the write is awaited like the marker ops. No signature change. **`writePendingNameSeed` is internally try/caught like `writeRecoveryOutcome` (`recovery_outcome.dart:59-78` — "losing the marker costs a notice, never the flow") (Gate round-4 adversary [P2])**: a throwing SharedPreferences write must not reject `surfaceRecoveryOutcome` and suppress the shipped #839/#458 restore snackbar + Sentry capture. Losing the breadcrumb costs the heal, never the notice. Call-site note: `uid` is statically `String?` there — `uid!` is safe once `expected != null` (the unreadable and mismatch arms have returned).
- After `surfaceRecoveryOutcome` completes, `recoveryOutcomeNoticeProvider` bumps `deviceNameSeedRevisionProvider` (`StateProvider<int>`, watched by the heal) so the heal re-evaluates EXPLICITLY once the breadcrumb lands, instead of relying on a groups emission happening to arrive after the post-frame prefs write (Gate round-3 adversary [P3] hardening — the implicit ordering was practically safe but emission-dependent). Note the bump lands only after the full notice completes (the `hasData` probe is bounded at ~4s), so worst-case the set-name chip is visible those few seconds before the heal fires — cosmetic, and the persistent breadcrumb remains the real safety net.
...
- **All side effects run inside `WidgetsBinding.instance.addPostFrameCallback` (Gate round-4 rubric [P2])** — mirroring `recovery_outcome_notice_provider.dart:67-71`. Setting `deviceNameSelfHealInFlightProvider.notifier.state` or clearing the breadcrumb from another provider's synchronous build body is illegal in Riverpod 2.x (StateError at boot); the trigger evaluation, in-flight check-and-set, housekeeping clears, and fetch launch all live in the post-frame callback. The `!cacheIsolationProvider` gate runs FIRST inside that callback, before any housekeeping — no breadcrumb mutation happens while isolated.
- Group iteration order is `createdAt DESC` (`watchUserGroups`, `group_provider.dart:552-566`) — the seeded name is deterministically the user's own member-doc name in their NEWEST qualifying group, stable across boots (not arbitrary).
- Expected cosmetic: on the heal boot the home set-name chip may render for a frame (or up to the ~4s probe bound) before the seed flips the greeting — self-correcting, not a bug.
