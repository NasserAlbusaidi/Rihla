...
- Written at the verified-success point: after the `expectedUid != null && uid == expected`-passing path is reached (i.e. NOT on mismatch, NOT on `!outcome.ok`, NOT for `opSignOut`, NOT when `expected == null` — a legacy/unverifiable marker gets no breadcrumb, conservative), synchronously before the async `hasData` probe. The probe outcome does NOT gate the write — the probe only picks snack copy.
...
- Watches `settingsProvider.select((s) => s.deviceName)`, `userGroupsProvider` (unwrapped as `valueOrNull ?? []` — `AsyncLoading` and the #647 false-empty both fail toward not-firing), and `isDurableUserProvider`; reads `sharedPreferencesProvider` for the breadcrumb.
- Trigger condition: `breadcrumb != null && breadcrumb == ref.read(currentUserIdProvider) && deviceName.trim().isEmpty && isDurable && groups.isNotEmpty && !ref.read(cacheIsolationProvider)`.
- Breadcrumb housekeeping inside the trigger evaluation: `deviceName` non-empty with a breadcrumb present → `clearPendingNameSeed`, stop; breadcrumb uid ≠ current uid → `clearPendingNameSeed` (stale from an older swap), stop.
...
9. Breadcrumb uid ≠ current uid → fetch never invoked, breadcrumb cleared (stale-swap pin).
