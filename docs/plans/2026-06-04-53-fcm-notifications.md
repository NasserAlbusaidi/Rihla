...
- **G1 — gate the notify on an ACTUAL join (the join tx is idempotent).** The tx skips `arrayUnion` when `memberIds.includes(uid)` (`:304`) and skips the member-doc `set` when `memberSnap.exists` (`:312`), yet still returns success on a no-op re-join. A second tap of an invite link (or any committed-join client retry) would otherwise re-notify every member of a join that didn't happen. Capture `let didJoin = false` in the outer scope and set `didJoin = !memberSnap.exists && !memberIds.includes(uid)` **inside the tx** — a BRAND-NEW member requires BOTH the member-doc create (`:312`) AND the memberIds arrayUnion (`:305`) to fire. Notify only when `didJoin` is true. (Outer-var write inside the tx is last-run-wins on Firestore retry — standard; the value reflects the committed run.) **Two-gate resolution (Gate R2 P2):** requiring both gates means a heal-path re-join (uid already in `memberIds` but member-doc missing — an existing repair path) does NOT re-announce, and the inverse split-state doesn't either. Only a genuinely new member (absent from both) triggers a notification — no false "X joined" on a heal.
