...
- **This fix INTRODUCES a new live-but-admin-less state that the codebase mostly AVOIDS.** Corrected precedent (Gate round 1 — my earlier claim was inverted): `deleteAccount.ts:612-628` does creator **succession** — on a creator's account deletion it reassigns `createdBy` to `oldestRealMemberUid` (`:205-214`), and only when there is NO real survivor does it tombstone `createdBy` AND soft-delete the whole group. So on account deletion the code *usually* avoids a live admin-less group; the conjunct fix is the first path that leaves one on a plain *leave*. **Caveat (Gate round 2):** `oldestRealMemberUid` filters `userId!==uid && !isTombstone && !isDeleted` but does NOT exclude `isShadow`, so succession can appoint a shadow uuid as `createdBy` — which under the new rules is also admin-less (uuid ∉ `memberIds`). Pre-existing and uncontrollable pre-fix (no live uid ever equals a shadow uuid), so it doesn't block this PR, but the succession follow-up (Task 5) must make `oldestRealMemberUid` exclude shadows/non-members.
...
**Files:** `functions/test/callables/deleteGroup.test.ts`, `removeMember.test.ts`, `addShadowMember.test.ts`, `claimRequest.test.ts` — read each file's existing harness/fixtures first and mirror them. One new test per callable: seed the FULL happy-path fixture (so the call would SUCCEED pre-fix), then mutate ONLY `group.memberIds` to drop the creator's uid while keeping `createdBy == uid` → invoke as that uid.

**CRITICAL (both round-2 reviewers, cross-confirmed): membership must be the SOLE failure cause, else the RED is red-for-the-wrong-reason and won't catch a future conjunct removal.** Each callable checks its creator gate BEFORE other preconditions, so an under-seeded fixture fails pre-fix with the WRONG error and a bare `assertFails` passes vacuously. Two guards on every RED test:
1. **Assert the SPECIFIC error code** — `expect(...).rejects.toMatchObject({ code: 'permission-denied' })` (or the harness's equivalent), never a bare throw/`assertFails`.
2. **Seed a state where the callable would otherwise SUCCEED** so the ONLY thing standing between it and success is the membership conjunct: `deleteGroup`/`removeMember` need a **net-zero-balance** group (else pre-fix `failed-precondition`, post-fix still `failed-precondition` — no signal); `decideClaimRequest` needs a **valid seeded pending claim request** (else pre-fix `not-found` at `decideClaimRequest.ts:226` — no signal); `addShadowMember`/`listGroupClaimRequests` have no such extra precondition but still assert the specific code.

Expected RED: pre-fix each call throws NO error / succeeds → the `permission-denied` assertion FAILS. Post-fix each throws exactly `permission-denied`. Commit RED.
...
3. File follow-up (HIGH priority, not a nicety): **creator succession on leave** — mirror `deleteAccount.ts:612-628`'s `oldestRealMemberUid` succession inside `leaveGroup` (reassign `createdBy` on creator-leave; tombstone + soft-delete only when no real survivor), AND fix `oldestRealMemberUid` (`deleteAccount.ts:205-214`) to also exclude `isShadow` docs so succession never appoints a non-member shadow uuid as creator. This closes the availability gap this PR opens (permanent admin-less lockout in the creator-leaves-then-deletes-account path, plus the shadow-succession edge). Reference this spec's Consequences section.
...
Round 2 (2026-07-11): rubric 0 P1 / 1 P2 / 2 P3; adversary 0 P1 / 2 P2 / 2 P3 — **both P1-clean in the same round, Gate PASSED.** Rubric independently re-verified all 5 rules RED tests as red-pre/green-post, both exclusion claims (correctSettlement/correctLogicalSettleUp upstream membership gate; dead `isGroupCreator`), and full createdBy-class completeness. Cross-confirmed P2 folded: callable RED tests must assert the SPECIFIC `permission-denied` code AND seed a would-succeed state (net-zero group / valid pending claim) so membership is the sole failure cause. P2 folded: `oldestRealMemberUid` shadow-succession edge (softens the "never admin-less" claim; added to the succession follow-up). P3: "5 callables" → 6 callables / 7 sites.
