...
- **#1141:** membership-scoped event, expense, event/group-settlement, member-join, and claim-request Branch A senders use stale recipient snapshots without a fresh delivery-time membership intersection. The follow-up requires a fail-closed current-membership lookup immediately before those sends, while preserving only claim decision Branch B's intentional cross-membership requester delivery and explicitly acknowledging the remaining leave-after-final-lookup race.
