...
- **#1144:** departure keeps historical event participants for balance conservation, but the expense editor also uses that retained list for NEW global/custom-fallback allocations. A member who leaves square can accrue fresh, unreadable debt. The follow-up separates the historical balance universe from future expense-recipient eligibility across client and rules contracts.
