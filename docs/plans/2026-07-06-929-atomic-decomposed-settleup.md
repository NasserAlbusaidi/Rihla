...
**Status:** GATE CLEARED — R2 (rubric 0 P1 + adversary 0 P1, same round; 2 rounds, 4 reviewers). Approved for implementation.
...
- The screen's pre-gate gains one clause: `decomposition.perEvent.length > kMaxDecomposeLegsAtomic` → the EXISTING fallback (`_recordSettlement`, one atomic single-doc group settlement — same path, same UX as the departed-member fallback; untagged, no ledger bump, both correct for a group-only write).
- **Honest framing (R2 adversary P2): the N>9 fallback knowingly reintroduces #752's SYMPTOM for that slice** — aggregate exactly correct (the oracle folds group settlements globally) but per-event ledgers keep showing the unsettled debt. This is a deliberate, bounded carve-out to CLAUDE.md's "don't simplify the decompose back to a single group write" contract, accepted because the atomic alternatives are worse (chunking reopens partial-persist offline; blocking the user is hostile). **This PR amends the CLAUDE.md #752 paragraph** to name the N>9 carve-out so the fallback isn't read as a contract violation later. The cap is conservative (2N+1 assumes zero cross-op caching; Firebase's same-doc caching may make the real budget ~N+1, i.e. N∈[10,19] might work) — widening it later requires emulator evidence, not doc optimism.
...
- Extract the `data` map literal of `addSettlement` (:89-109) into `static Map<String, dynamic> buildSettlementDoc({required String id, required String eventId, required String payerParticipantId, required String recipientParticipantId, required Decimal amount, required String createdBy, required String currency, String? payerName, String? recipientName, String? note, String? groupSettleUpId, required DateTime settledAtUtc})` — mirror :91-109 FIELD-FOR-FIELD (14 keys: id, eventId, payerParticipantId, recipientParticipantId, payerName, recipientName, amountFils, currency, note, isDeleted, deletedAt, settledAt, createdBy, + groupSettleUpId omitted-when-null). `addSettlement` calls it (behavior byte-identical); `buildGroupSettlementDoc` mirrors `group_settlement_service.dart:77-96` the same way (adds groupId, `eventId: groupId` sentinel, `scope: 'group'`). Test 5's builder byte-equality assertion pins both (R2 rubric P3 — exact keys, no ellipsis).
...
```
  ...
  - The activity log (`logGroupEvent`, screen :876-894) stays a separate fire-and-forget write OUTSIDE the batch — pre-existing non-atomic display-only behavior, unchanged; at offline replay the batch can reject while the activity entry commits. Texture change, named (R2 adversary P3): such an orphaned "settled X with Y" feed row is now backed by ZERO money docs where the sequential path left it partially backed — display-only, never feeds balances. This PR does NOT extend atomicity to the activity log (R1 rubric P3 — stated so it isn't mistaken for a new guarantee).
...
Fix axis = write atomicity. Orthogonal example 1 (time × connectivity): user records a decomposed settle-up OFFLINE, then the counterparty is REMOVED from an event before reconnect. Today: event legs for still-valid events commit at replay, the removed-event leg rejects → event ledgers show a partial settle-up that the aggregate never matched. Post-fix: the whole batch rejects at replay → nothing persists; balances stay pre-settle-up-consistent; user re-enters when online. Named trade-off (accepted): the failure mode changes from *silent partial persist* (money-wrong, unrepairable without manual offsetting rows) to *near-silent no persist* (re-enter). Replay-rejection observability (R2 rubric P3 correction — v1 wrongly claimed no channel exists): `awaitServerAck`'s `observeLate()` (`write_ack.dart:38-56`) already reports late/replay failures to Sentry + `notifyLateWriteFailure`; racing the single batch-commit future through it yields ONE report per logical settle-up instead of today's N+1 — strictly better, and no new channel is built here.
...
6. **Emulator pins** (new `functions/test/decomposed-settleup-batch.test.ts` — kebab-case matching the existing `firestore-rules-publish-readiness.test.ts`, whose helpers it reuses; there is no `*.rules.test.ts` convention in this repo): (a) an all-valid batch of 2 event settlements + 1 group settlement (client shapes verbatim) commits; (b) same batch with ONE leg's counterparty not an event participant → the ENTIRE commit rejects, zero docs exist afterward; (c) an all-valid batch of 9 event settlements (9 distinct events) + 1 group settlement commits. **What 6a-c pin (R2 rubric P2): batch SHAPE and per-op rule LOGIC — NOT the access-call budget.** The rules emulator does not reliably enforce the 10/20 document-access-call limits (distinct from the 1000-expression ceiling it DOES enforce), so no test can empirically pin the prod budget; `kMaxDecomposeLegsAtomic = 9` rests on the verified `2·N+1` arithmetic (§ceiling — per-leg doc reads re-counted from the rules functions; `validSettlementCore` has zero `get()`s). Every wrong-cap outcome remains money-safe: online → throw, nothing persists; offline → silent no-persist; never partial, never wrong money. NO >cap rejection pin (cache-dependent, unstable). Run: `cd functions && npm run test:emulator -- decomposed-settleup-batch.test.ts` (runner forwards args).
