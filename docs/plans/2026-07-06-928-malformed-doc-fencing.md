...
| `amountFils` | `as int?` (non-int) | `is int ? v : 0` | oracle `typeof === 'number' ? v : 0`; JS-double divergence accepted — client zeroes ONE doc's amount, corrupts no one else's shares; rules pin `positiveInt(data.amountFils)` on the expense create path (firestore.rules:761; :108 is the settlement pin `validSettlementCore`) so only Admin/legacy docs can differ. Named for symmetry with the payer case: BOTH are bounded to the same Admin/forged/legacy class. Precise blast radius (R2 rubric P3): the client zeroing what the oracle counts shifts co-participants' nets between the online aggregate and the offline once-path, bounded to that one expense's amount; accepted because no clean Dart-int salvage can match JS-number semantics and the class is rules-unreachable |
| `currency` | **`as String?` cast THROWS on non-string** (R2 rubric P1 — the #47 fence covers only unsupported-STRING/absent) | `data['currency'] is String ? … : 'OMR'` wrapping the existing `isSupported` check | EXACT mirror of `currencyOf(raw: unknown)` :52-54 (`typeof === 'string' && supported ? raw : 'OMR'`) — the oracle COUNTS a `{currency: 42}` doc as OMR; the client must too, not throw-then-skip |
...
| `currency` | **`as String?` cast THROWS on non-string** (R2 rubric P1, same as expense) | `data['currency'] is String ? … : 'OMR'` wrapping the existing `isSupported` check | exact `currencyOf` mirror, as expense |
...
**Named residual (accepted):** a doc whose STORED payer is the literal string `''` — the oracle's `typeof` check passes and seeds a phantom `''` row server-side while the guarded client drops it. Unreachable by any rules-valid write (`payerParticipantId in participants()` — `''` cannot be a participant), i.e. Admin/forged-only. Divergence direction, stated precisely (R2 adversary P3): the oracle's inflated divisor LOWERS co-participants' server-side owed, so the guarded client shows them owing MORE than the aggregate doc — visible only while such an Admin-forged doc exists. Accepted on unreachability, not on direction. Symmetric to the amountFils-double acceptance above.
...
| `cross_group_activity_pager.dart` `_fetchOne` :121-130 | bare `.map` in per-group catch (one bad row silently drops the whole GROUP from the merged feed) | helper, context `'CrossGroupActivity.fetch'` (keep the existing sort chained after) — **PLUS the frontier fix (R2 adversary P1):** `_fetchNeeded`'s fold computes `exhausted: o.logs!.length < kCrossGroupActivityPageSize` from the DESERIALIZED count; once the fence can skip rows, a FULL raw page (50) with 1 malformed row yields 49 → the group is falsely marked exhausted and `hasMore`/`needsFetch` (`merged_activity_paginator.dart:62-71`) never fetch its remaining docs — silent truncation of that group's older activity. Fix: compute exhaustion from the RAW count — `_FetchOutcome` gains `rawCount: snap.docs.length` (set alongside `logs`), and the fold uses `exhausted: o.rawCount < kCrossGroupActivityPageSize`. The other two pagers are already raw-count-safe (`activity_feed_screen.dart:105`, `group_activity_screen.dart:116` — verified, `snap.docs.length`) and need no change |
...
- `watchExpenses` → `eventExpensesProvider` → ledger list + `groupBalancesProvider` (in-group balances). Derived exports (Trip Receipt `trip_receipt_builder.dart:68`, Event Recap) read the same fenced providers — a salvaged garbage-`createdAt` expense renders a "1 Jan 1970" receipt/CSV row: display-only honest-garbage, accepted (R2 adversary P3).
...
5b. **Pager frontier under a FULL page** (R2 adversary P1 — the short-page shape of test 5 cannot catch this): seed one group with `kCrossGroupActivityPageSize` docs where ONE mid-page doc is malformed, plus more docs beyond the page → after page 1 (49 decoded of 50 raw) the group is NOT exhausted and the next fetch surfaces the older docs. RED against the naive fence (group falsely drained); GREEN with `rawCount`-derived exhaustion.
...
7. **Factory-totality guardrail (R1 adversary P2)**: one fixture per money model with EVERY field simultaneously wrong-typed — **explicitly including `currency: 42`** (R2 rubric P1 — the field the v2 table wrongly exempted), int strings, map numbers, list maps… → `fromFirestore` returns a salvaged object, never throws. This is the standing pin that keeps the money fence a dead branch. Enumerate the fixture's keys from the model type, not from the salvage table.
...
- Modify: `lib/features/activity/screens/activity_feed_screen.dart:_loadPage`, `lib/features/groups/screens/group_activity_screen.dart:_loadPage`, `lib/features/home/providers/cross_group_activity_pager.dart:_fetchOne` (+ the `rawCount` frontier fix), `lib/features/groups/services/group_activity_service.dart` (2 sites, :47/:77).
