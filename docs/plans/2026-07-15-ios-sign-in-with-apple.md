...
Changes:
- `_conflict`/`_conflictShellGateOwner` widen to `LinkConflictException?` — and so does the `_conflictShellEmpty(GoogleLinkConflictException conflict)` parameter at `durable_credential_sheet.dart:172` (Gate R3 P3; compile-caught, listed for completeness).
...
4. Manual on-device smoke once the portal/console checklist is done — record in the PR body. Must include the exact sequence App Review runs for 5.1.1(v): **link Apple → create data → delete account → re-sign-in with the same Apple ID → verify zero data comes back** (plus: link, conflict-switch, restore, cancel-at-every-prompt leaves the shell intact). Two Gate R3 P3s to confirm specifically on-device: the conflict-switch's reuse of the failed-link Apple credential actually signs in (the "no second sheet" claim is inferred from Google parity; Apple tokens are finickier — failure is non-destructive but would need a fallback re-prompt), and the Arabic `SignInWithAppleButton` label renders acceptably (the package hardcodes `.SF Pro Text`; no RTL mirroring).
...
- **R3 (2026-07-15): rubric 0 P1/0 P2/3 P3; adversary 0 P1/0 P2/3 P3 — BOTH CLEAN IN THE SAME ROUND: Gate PASSED.** P3 notes folded in (widen `_conflictShellEmpty` param; thread `hasAppleProvider` through the static `show()`; on-device confirms for conflict-credential reuse + Arabic Apple button; `appleAccountTile` rendering for an Apple-linked account restored on Android is the one deliberate, stated deviation from "zero Android visual change" — unreachable for today's Android users). Round count note: R2's P1 was a self-consistency defect introduced by the R1 revision (D6 declared keys with no wiring task), not new scope — the spec was not split.
