...
- Client authority affordances key on the **group doc**: `group_settings_screen.dart:45` (`isCreator = currentUserId == group.createdBy`), `group_detail_screen.dart:261,265,1176`. Live-watched → succession propagates with zero client change.
...
- **S6 — non-member doc never appointed (defense-in-depth):** survivors = a NON-shadow doc with `userId: 'ghost'` NOT in memberIds (torn state), `joinedAt` EARLIER than MEMBER's (explicit — the pin is vacuous if the ghost sorts last anyway) + MEMBER (in memberIds). OWNER leaves → `createdBy == MEMBER`. Pins conjunct (e) on the leaveGroup side; keep `isShadow: false` on the ghost doc so (d) can't mask it.
...
- **Round 2** (new fresh pair): rubric **0 P1 / 0 P2 / 3 P3**, adversary **0 P1 / 0 P2 / 2 P3** — **both P1-clean in the same round → Gate closed; implementation approved.**
  - Rubric independently re-verified: tx reads-before-writes at both sites; shadow-in-memberIds (the R1 P1) genuinely fixed; no existing test regresses (the only `role:'MEMBER'` assertion is on a tombstone doc); `MEMBER_BALANCE_KEYS=['userId','isTombstone']` means the role flip fires ZERO aggregate recompute; no trigger watches the bare group doc.
  - Adversary independently re-derived the never-authenticates mechanism; audited every deleteAccount fixture (no survivor is ever seeded outside memberIds → conjunct (e) breaks nothing); l10n clean (`groupRoleCreator` EN/AR pair exists, no new strings).
  - [P3s applied] S6 now explicitly requires the ghost's `joinedAt` EARLIER than MEMBER's; `group_detail_screen.dart` line refs reconciled. [P3s acknowledged, not actioned] no `creator_changed` activity row (declared out of scope); roster badge not necessarily first row under an older shadow (pre-existing cosmetic class); torn-doc soft-delete trade-off (documented in Consequences, D3-pinned).
