...
- `deleteAccount.ts:310-319` — `oldestRealMemberUid(members, uid)` filters `userId !== uid && isTombstone !== true && isDeleted !== true`, sorts by `timestampMillis(joinedAt)`. **No `isShadow` conjunct, no memberIds conjunct** — succession can appoint a shadow uuid (never in `memberIds`, never authenticates) → admin-less under the #1132 membership-conjunct rules. This is the Gate-round-2 catch recorded in `docs/plans/2026-07-11-1132-departed-creator-authority.md` Consequences ("the succession follow-up must make `oldestRealMemberUid` exclude shadows/**non-members**").
...
- `addShadowMember.ts:124-135` — shadow member doc: `userId` = freshly-minted uuid (**not** added to `memberIds`), `isShadow: true`, `joinedAt: serverTimestamp()`. So a shadow can carry the OLDEST `joinedAt` among survivors.
...
Move (verbatim) `timestampMillis` from `deleteAccount.ts:159-167` and `deletedUserSentinel` from `deleteAccount.ts:25`; add the successor helper with the two new conjuncts:
 ...
 * tombstone, (c) not soft-deleted, (d) NOT an unclaimed shadow — a shadow's
 * uuid is never in memberIds and never authenticates, so appointing it as
 * createdBy is admin-less under the #1132 membership-conjunct rules — and
 * (e) a CURRENT member (userId ∈ memberIds): a torn/legacy doc whose userId
 * fell out of memberIds would be equally admin-less.
 */
...
`deleteAccount.ts`: delete the local `timestampMillis`, `deletedUserSentinel`, and `oldestRealMemberUid`; import all three from `./shared/membership`; Phase C call becomes `oldestRealMemberUid(members, uid, currentMemberIds)`.

Note on the memberIds argument: both callers pass the **pre-departure** roster (`currentMemberIds` / `freshMemberIds`) — the departing uid is excluded by conjunct (a), and `deleteAccount`'s tombstone-id swap into memberIds is harmless (tombstone docs are excluded by conjunct (b)).
...
- **The shadow-exclusion fix changes `deleteAccount` behavior** for the "only shadows survive" case: previously appointed a shadow uuid (admin-less), now soft-deletes the group (`orphaned: true` in the cascade result). Strictly better; no test pins the old behavior.
...
3. **Read-path per write-path** — `createdBy`: rules gates + client `isCreator` derivations (named above). `role`: roster badge `group_members_section.dart:163,169`. `isDeleted`/`deletedAt`: quiesce checks in all three membership callables + client stream filters.
...
Six tests (S1-S6). Seeds use the existing `seedGroup`/`seedMember`; shadow docs seed with `docId: uuid, userId: uuid, isShadow: true` and the uuid NOT in `memberIds`.
...
- **S3 — shadow never appointed:** survivors = one unclaimed shadow (joinedAt `2026-01-01`, OLDER than real MEMBER `2026-01-02`) + MEMBER. OWNER leaves → `createdBy == MEMBER`, shadow doc role untouched.
- **S4 — no real survivor → soft-delete:** group `[OWNER]` + one unclaimed shadow doc. OWNER leaves → group `isDeleted == true`, `deletedAt` set, `createdBy == 'deleted-user'`, shadow doc still present (mirror deleteAccount: no doc cleanup).
- **S5 — non-creator leave unchanged:** MEMBER leaves → `createdBy == OWNER`, no role flips, no soft-delete.
- **S6 — non-member doc never appointed (defense-in-depth):** survivors = a doc with `userId: 'ghost'` NOT in memberIds (torn state) + MEMBER (in memberIds, later joinedAt). OWNER leaves → `createdBy == MEMBER`.
...
- **D1 — shadow-only survivor soft-deletes:** creator-owned group, memberIds `[deletedUid]`, one unclaimed shadow member doc. `deleteAccount` → group `isDeleted == true`, `createdBy == 'deleted-user'`, result `groupOrphanedAndSoftDeleted == true`. (Currently RED: shadow uuid gets appointed, group stays live.)
- **D2 — older shadow loses to newer real member:** shadow joinedAt `2026-01-01`, real `otherUid` joinedAt `2026-01-05`, both docs present, memberIds `[deletedUid, otherUid]`. → `createdBy == otherUid` AND `otherUid`'s doc `role == 'CREATOR'` (role-flip parity; currently RED on both).

**Step 2: Run to verify they fail**

Run: `cd functions && npm run test:emulator -- deleteAccount.test.ts -t "#1138"`
Expected: D1 FAILS (`isDeleted` false, `createdBy` = shadow uuid), D2 FAILS (`createdBy` = shadow uuid).
