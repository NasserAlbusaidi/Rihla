...
- **ADD soft-delete carve-out (the trap):** a soft-delete update (`isDeleted:true`, `deletedAt`, `lastEditedBy:caller`; no `currency` in the diff) on an OMR expense in an OMR group → still **assertSucceeds** (proves the diff-gate didn't break append-only soft-delete).
