...
1. **"Outstanding" is defined per currency bucket: ∃ participant with `netBalance != Decimal.zero` in that bucket.**
   Rationale: available identically at both scopes (the group provider has nets, not optimizer output), conservative (any imbalance keeps the warning), and equivalent to "optimizer emits ≥1 transfer" in all conserving cases. The issue's "stronger fix" (per-expense contribution filtering) is REJECTED: an individual expense's contribution is not well-defined after netting — a settled-out expense still shaped the net. Bucket-zero is the crisp, explainable signal.
...
4. **Fail-open on errors (money-trust: prefer a spurious warning over a hidden one).**
   ...
   - Group scope, a single event's settlement stream `hasError && !hasValue`: treat that event's settlements as empty and keep going (decision 3's gate only blocks on *loading*).
