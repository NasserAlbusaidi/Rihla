...
- "bell tap clears the unread dot": seed unread state (pattern from `activity_unread_test.dart`), tap bell → `HomeKeys.activityUnreadBadge` `isLabelVisible == false`.
