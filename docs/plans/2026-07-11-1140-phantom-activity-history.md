...
> The 32-unit `substring` clamps on UTF-16 code units (matching the rules' `size()` and the whole codebase's UTF-16 name convention — CLAUDE.md "don't use `runes.length`"). A clamp that split an astral surrogate leaves a lone surrogate — still rule-valid (`isValidDisplayName` accepts it) so it **never blocks the batch**; at worst a cosmetic half-glyph, and only in the unreachable-today empty-`deviceName` settle-on-behalf fallback with a 33+-unit former-member name. Accepted (Gate R2/R3 P3); no rune-boundary clamp needed.
```dart
...
> **MANDATORY test-env setup (Gate R3 P1):** `event_danger_section.dart:497` reads `actorId` from `FirebaseConfig.currentUser?.uid` (NOT an overridable provider). With **no signed-in user** (the default in `event_settings_screen_test`), `actorId == ''` → the D7 domain-only fallback fires → **no activity leg is ever staged**, making every event-delete activity assertion vacuous/impossible. So EVERY event-delete test that exercises the activity path MUST register a fake `FirebaseAuthPlatform` with a signed-in uid — template: `test/core/services/display_name_propagation_service_test.dart` (#1124; needs `firebase_auth_platform_interface` as a direct dev_dependency, each file is its own isolate). Production is unaffected (anon auth guarantees a non-null `currentUser`).

**Step 1 — Write failing test (RED):** register the fake signed-in `FirebaseAuthPlatform` (uid `uid-alice`). One `FakeFirebaseFirestore`; a real `GroupActivityService.withFirestore(fake)`; mock `EventService.deleteEvent` to return a pending-then-`permission-denied` `Completer` (offline queued → denied). Drive the delete confirm. **Assert `groups/{gid}/activity` is empty** after the denial.
Expected today (pre-fix): FAIL — a doc exists (the screen's separate `logGroupEvent` runs before/independently of the delete). Post-fix the screen passes the activity INTO `deleteEvent` (whose mock eats the batch), so no separate row → GREEN. This flip is only meaningful WITH the signed-in uid (else post-fix takes the domain-only path and the test passes for the wrong reason).
...
**Step 4 — Run, expect PASS** + add TWO more tests (all with the signed-in fake auth):
- **GREEN twin (success → exactly one row):** real `EventService.withFirestore(fake)` + signed-in uid → confirm the delete succeeds and `groups/{gid}/activity` holds exactly one `evt_deleted_<eventId>` row with the expected `actorId`/`metadata` (proves the batch writes both atomically).
- **D7 fallback (no uid → domain-only, still routes):** NO signed-in auth (or a throwing `currentUser`) → assert `deleteEvent` is called **without** activity params and the screen still routes to `/group/$groupId` (preserves the D-14 "delete never blocked by activity" coverage that the old `throwOnLog` test loses).
...
> Guard note (Gate R4 P3): `stageEvent`'s activity params are valid ONLY for existing-group creates (the rules' standard `validEventCreate` branch, L561, has `isGroupMember`). NEVER wire them into a `groupFoundingBatchByCreator` write — there the group doc doesn't exist pre-batch so the activity op's `isGroupMember` reads false and would deny the entire group-creation batch. Founding hand-builds its seeded event inline (`group_provider.dart:236`) and correctly does NOT call `stageEvent`; keep it that way.
...
**The completeness mechanism is `flutter analyze` + grep, NOT this file list.** Every Class-B break is a deterministic `invalid_override` compile error — so `flutter analyze` (Task 7 Step 1) mechanically surfaces *all* of them, including any file this list misses; the enumeration below is the known-instances starting point (2026-07-11), not an exhaustive authority. Class-A breaks surface as test failures + the grep. Run before porting: `grep -rln "logCalls\|logGroupEvent\|_Recording\|_Failing\|_Denied\|verifyNever\|verifyZeroInteractions\|addSettlement\|deleteEvent(\|stageEvent(\|addGroupSettlement\|stageDecomposedSettleUp" test/` and cross-check every hit is either addressed or provably unaffected (passes no activity params → legacy path).

**Files (enumerated — confirm line numbers at port time; counts are 2026-07-11 grep hits):**
- `test/features/ledger/settle_up_screen_test.dart` — **Class B**: `_RecordingSettlementService:2170`, `_FailingSettlementService:2220`, `_DeniedSettlementService:2242` each override `addSettlement` → add the optional activity params to each signature (behavior unchanged: record/throw/deny still fire on the batched call). The offline-412 mock at L1482 (`() => service.addSettlement(...)`) — re-point to the new signature. The `#831` activity-presence assertion (~L781-789, reads `fakeDb…/activity`) must run against a **real** `SettlementService.withFirestore(fake)` so the batched activity row lands and is readable.
- `test/features/ledger/settle_up_amount_error_test.dart:88-95` and `test/features/ledger/settle_up_revalidation_test.dart:92-100` — **Class B (Gate R3 P2)**: both declare `_RecordingEventSettlementService extends SettlementService` overriding `addSettlement`; add the optional activity params to each override signature (else `invalid_override`). Behavior (record/assert-nothing-written) unchanged.
- `test/features/events/event_settings_screen_test.dart:558-631` — **Class A+B**, and **register the signed-in fake `FirebaseAuthPlatform`** (Gate R3 P1) so the activity path is taken. (1) L586-588 `verify(deleteEvent(groupId,eventId))` → extend the `when(...)` stub (L564-569) and the `verify` with the 5 new `any(named:)`/expected args. (2) L590-596 `activityService.calls.single.type=='event_deleted'` → the activity is now a `deleteEvent` PARAM, not a `logGroupEvent` call: assert via `verify(() => service.deleteEvent(..., activityId: 'evt_deleted_${event.id}', activityActorName: 'Test User', activityMetadata: {'eventId':…,'eventName':…}, activityActorId: any(named:'activityActorId')))`. (3) L599-631 `throwOnLog:true` "logging failure doesn't block delete" → **re-purpose** to the D7 fallback (see Task 2 Step 4): with NO signed-in auth, assert `deleteEvent` is called **without** activity params and the screen still routes back — preserving the D-14 coverage the old premise loses.
