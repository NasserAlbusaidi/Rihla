...
- `tool/first_100_launch_packet.dart` to generate the private first-10 outreach
  messages, send sheet, and checklist in one step.
...
- `tool/first_100_tracker_patch.dart` to apply the private roster to
  `~/Desktop/rihla-first-100-tracker.csv` after sending the first batch while
  keeping champion names out of git.
...
For the first 10 private champions, prefer one launch packet so the outreach
...
link. Champions install Rihla straight from the public Play listing via that
tracked link - there is no tester list to upload and no opt-in step to send.

Run `tool/first_100_tracker_patch.dart --mark-contacted` only after the batch is
actually sent. The private tracker can contain champion names, but it must stay
outside git.

When a champion confirms they installed Rihla from the public Play listing,
re-run the patch with `--mark-tester-added` for their slot. The `tester_added`
column now records a confirmed Play install, not a closed-test tester add:
...
```text
I just released Rihla on Google Play.
...
```text
أطلقت تطبيق Rihla على Google Play.
...
```text
Split dinner with your group without signup. Rihla is on Google Play:
https://rihla-safar.web.app/?utm_source=instagram&utm_medium=video&utm_campaign=first_100&utm_content=legacy_dinner
```
...
- Fix any install or first-run failures from Day 1.
...
- Do not commit the private champion roster (real names) to git.
