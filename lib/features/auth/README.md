...
### services/
- **auth_recovery_service.dart**: Orchestrates the email-link + Google link/send/restore flows. Methods: `linkEmailToCurrentUser`, `sendRecoveryLink`, `completeEmailLink`, `linkGoogleToCurrentUser`, `restoreWithGoogle`, `restoreWithEmailLink` (no-merge cross-UID discard-shell swap, #441 PR4), `signOutCurrentDevice`, plus pending-email and in-flight-op (`opLink`/`opRecover`) SharedPreferences helpers. Uses Firebase Auth's email-link mechanism with App Links / Universal Links continue URLs.
...
- **recover_screen.dart**: Enter-email screen for the slim email-fallback restore (home empty-state secondary CTA); calls `sendRecoveryLink`.
