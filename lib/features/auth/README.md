...
- **uid_change_listener.dart**: Watches `auth.userChanges` and wipes the local SQLite cache via `LocalDatabase.wipeAndReinitialize()` whenever the active UID changes. Critical for account recovery — prevents anonymous data from leaking across UIDs.
