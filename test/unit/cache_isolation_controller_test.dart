...
void main() {
  ...
  group('markFirestorePersistenceDirty', () {
    test('sets the dirty flag true so the next boot reconcile clears', () async {
  ...
  group('PlatformCacheIsolationController', () {
    test(
      'engageIsolation flips the flag AND invalidates the leaf subscription '
      'providers (R5 P2-1)',
      () {
        ...
        final builds = <String, int>{'authLink': 0, 'notif': 0, 'bootstrap': 0};
        ...
        expect(builds, {'authLink': 1, 'notif': 1, 'bootstrap': 1});
        ...
        container.read(notificationServiceProvider);
        container.read(appBootstrapProvider);
        expect(builds, {'authLink': 2, 'notif': 2, 'bootstrap': 2});
      },
    );
