...
void main() {
  ...
  group('ParticipantCacheRepository', () {
    ...
    test(
      'cacheParticipants writes and getCachedParticipants reads them back',
      () async {
        ...
        await repo.cacheParticipants(ownerUid, 't1', [p1, p2]);

        final loaded = await repo.getCachedParticipants(ownerUid, 't1');
    ...
    test('returns empty list when nothing cached', () async {
      final loaded = await repo.getCachedParticipants(ownerUid, 'unknown-trip');
      expect(loaded, isEmpty);
    });

    test('caching an empty list clears prior entries for that trip', () async {
      await repo.cacheParticipants(ownerUid, 't1', [build(id: 'p1')]);
      expect(await repo.getCachedParticipants(ownerUid, 't1'), hasLength(1));

      await repo.cacheParticipants(ownerUid, 't1', []);
      expect(await repo.getCachedParticipants(ownerUid, 't1'), isEmpty);
    });
    ...
    test('rows for one trip do not leak into another trip query', () async {
      await repo.cacheParticipants(ownerUid, 't1', [
        build(id: 'p1', tripId: 't1'),
      ]);
      await repo.cacheParticipants(ownerUid, 't2', [
        build(id: 'p2', tripId: 't2'),
      ]);

      final t1 = await repo.getCachedParticipants(ownerUid, 't1');
      final t2 = await repo.getCachedParticipants(ownerUid, 't2');
      expect(t1.map((p) => p.id), ['p1']);
      expect(t2.map((p) => p.id), ['p2']);
    });

    test(
      'role string defaults to member if unknown — fromString fallback',
      () async {
    ...
    test(
      'participant with null userId, displayName, avatarUrl round-trips',
      () async {
        await repo.cacheParticipants(ownerUid, 't1', [
          build(id: 'p1', userId: null, displayName: null, avatarUrl: null),
        ]);
        final loaded = await repo.getCachedParticipants(ownerUid, 't1');
