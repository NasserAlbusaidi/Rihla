      ...
      GoRoute(
        ...
        routes: [
          GoRoute(
            path: 'sent',
            builder: (_, state) =>
                _SentScreen(email: state.extra as String? ?? ''),
          ),
        ],
      ),
    ],
...
Future<void> _typeAndSubmit(
  ...
  await tester.enterText(
    find.byKey(const Key('linkEmail.email')),
    email,
  );
  await tester.enterText(
    find.byKey(const Key('linkEmail.confirm')),
    confirm,
  );
...
void main() {
  ...
  testWidgets('happy path calls service with normalized email and routes to sent', (tester) async {
  ...
  testWidgets('credential-already-in-use surfaces the restore-from-other-account message',
      (tester) async {
    ...
    expect(
      find.byKey(const Key('linkEmail.error')),
      findsOneWidget,
    );
    expect(
      find.text(
