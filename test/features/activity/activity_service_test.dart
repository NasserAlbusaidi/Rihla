...
void main() {
  ...
  group('eventTransactionActivityProvider', () {
    test('filters to MONEY category only', () async {
      final db = FakeFirebaseFirestore();
      await _logsCollection(db).doc('a1').set({
        'id': 'a1',
        'eventId': _eventId,
      ...
      final logs = await container
          .read(eventTransactionActivityProvider(
            (groupId: _groupId, eventId: _eventId),
          ).future);
      expect(logs.map((l) => l.id), ['a1']);
    });
  });

  group('legacy tripActivityProvider', () {
    test('emits an empty list', () async {
      final container = ProviderContainer();
      addTearDown(container.dispose);
      final logs = await container.read(tripActivityProvider('t1').future);
      expect(logs, isEmpty);
    });

    test('tripTransactionActivityProvider also empty', () async {
      final container = ProviderContainer();
      addTearDown(container.dispose);
      final logs =
          await container.read(tripTransactionActivityProvider('t1').future);
      expect(logs, isEmpty);
    });
  });
}
